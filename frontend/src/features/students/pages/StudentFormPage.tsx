...
export function StudentFormPage() {
  ...
  const [isOfflineExisting, setIsOfflineExisting] = useState(false);
  ...
  const form = useForm({
    ...
    validate: {
      ...
      password: (value, values) => {
        if (values.isOffline || !value?.trim()) return null;
  ...
  const { isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const offline = student.hasPortalAccess === false;
      setIsOfflineExisting(offline);
      form.setValues({
  ...
  const createMutation = useMutation({
    mutationFn: async (values: typeof form.values) => {
      const { password, isOffline, email, ...rest } = values;
      const payload = {
        ...rest,
        centerId: rest.centerId || user?.centerId,
        isOffline,
        ...(email.trim() ? { email: email.trim() } : {}),
        ...(!isOffline && password.trim() ? { password: password.trim() } : {}),
      };
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: typeof form.values) => {
      const { centerId: _c, enrollmentDate: _e, dateOfBirth: _d, gender: _g, password: _p, isOffline: _o, ...rest } =
        values;
  ...
  const showOfflineCheckbox = !isEdit;
  const hideLoginFields = (!isEdit && form.values.isOffline) || (isEdit && isOfflineExisting);
          ...
          <Paper shadow="sm" p="lg" radius="md">
            <Grid>
              {showOfflineCheckbox && (
                <Grid.Col span={12}>
                  <Checkbox
                    label={t('students.form.isOffline')}
                    description={t('students.form.isOfflineHint')}
                    {...form.getInputProps('isOffline', { type: 'checkbox' })}
                  />
                </Grid.Col>
              )}
              {isEdit && isOfflineExisting && (
                <Grid.Col span={12}>
                  <Alert icon={<IconInfoCircle size={16} />} color="gray" variant="light">
                    {t('students.form.offlineReadonlyHint')}
                  </Alert>
                </Grid.Col>
              )}
              <Grid.Col span={12}>
              ...
              {!hideLoginFields && (
                <Grid.Col span={{ base: 12, sm: 6 }}>
                  <TextInput
                    ...
                    required={!form.values.isOffline}
                    {...form.getInputProps('email')}
                  />
                </Grid.Col>
              )}
              {!isEdit && !form.values.isOffline && (
                <Grid.Col span={{ base: 12, sm: 6 }}>
                  <TextInput
              ...
              {hideLoginFields && (
                <Grid.Col span={{ base: 12, sm: 6 }}>
                  <TextInput
                    label={t('students.form.emailOptional')}
                    placeholder={t('students.form.emailOptionalPlaceholder')}
                    {...form.getInputProps('email')}
                  />
                </Grid.Col>
              )}
